...
/* Hourly self-cleanup: rewrite members.txt as the current live snapshot,
 * dropping any row whose lease is no longer held.  Caller holds g_server_mutex
 * (it reads the lease tree); the file write takes g_file_mutex internally. */
